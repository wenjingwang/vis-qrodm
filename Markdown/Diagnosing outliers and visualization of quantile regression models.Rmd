...
Quantile regression models have been widely used in numerous applications, and the subject of substantial research. A review article (@koenker2017) to appear this year surveys the area over the past 40 years. One recommendation that he suggests is that more diagnostics should be provided for the models, to supplement the extensive toolboxes for estimation. This paper provides diagnostics for assessing the robustness of quantile regression models and implements outlier detection methods in the R language (@RCore). The R package `quokar` contains functions for diagnosing outliers in quantile regression and also provides supporting visualizations. In addition, this paper proposed a general framework to visualize quantile regression model (linear and non-linear) in high dimensional data space using `GGobi`. The integrated plot of fitted model and original data set provides another approach to examing outliers, and how well the model fits the data. 
