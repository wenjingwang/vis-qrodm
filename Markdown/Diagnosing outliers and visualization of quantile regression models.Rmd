...
add visualize qr model in ggobi

This article aims to introduce R package `quokar` and propose general framework of visualizing quantile regression models in high dimension. The remainder of this article is organized as follows: In Section 2, we provide a general introduction to quantile regression model and prove its robusness. In Section 3 we give a tour of outlier diagnostic methods used in package `quokar`. In section 4 we will show how to conduct diagnostic methods in package `quokar`. In section 5, we displayed supportive visualizations for quantile regerssion in linear and non-linear situations. The current limitations and future research and development directions are discussed in Section 6.
