---
title: "Diagnosing Outliers and Visualization of Quantile Regression Model"
author:
...
Quantile regression model has been widely used in numerious research areas. Recently, Koenker reviewed the studies of quantile regression in the past 40 years, in which he pointed out that more diagnostic tools should be provided apart from the extensive toolboxes for estimation. This paper first comprehensively examined the robustness of quantile regression and implement outlier diagnostic methods in R language. The R package `quokar` contains functions for diagnosing outliers in quantile regression and also provide supportive visualizations of the results. In addition, this paper proposed a general framework to visualize quantile regression model (linear and non-linear) in high dimensional data space using `GGobi`. The integrated plot of fitted model and original data set provide another straight-forward way of examing outliers, which also supply massive information about data and model on other aspects. This paper discusses diagnostic methods used in `quokar` and illustrates the usefulness of the package through examples. At last, the visualization framework and results are provided.

# Introduction

Quantile regression model has been widely used in many research areas such as economy, finance and social science. Most recent researches are discussed in @autor2017effect, @mitchell2017physical, @gallego2017corporate, @maciejowska2016probabilistic. Quantile regression has significant advantages over mean regression mainly on two aspects: (a) observed covariates can describe the whole distribution of response variable which produce comprehensive results; (b) estimators can still maintain optimal properties of in case of heteroscedasticity or heavy tail distribution.
...
```{r single-case, data-example, warning=FALSE,message=FALSE, fig.height=3, fig.width=5, fig.align = "center", fig.cap="Plot the outlier state for single variable case."}
